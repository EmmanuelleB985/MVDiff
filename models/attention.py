...
class EpipolarCrossAttention(nn.Module):
    ...
    def compute_epipolar_mask(
        ...
        coords = torch.stack([x.flatten(), y.flatten(), torch.ones_like(x.flatten())], dim=-1)
        coords = coords.float()
        
        # Compute epipolar lines: l = F^T * p
        epipolar_lines = torch.einsum('bij,nj->bni', fundamental_matrix.transpose(1, 2), coords)
        
        # Normalize lines
        epipolar_lines = epipolar_lines / (epipolar_lines.norm(dim=-1, keepdim=True) + 1e-8)
