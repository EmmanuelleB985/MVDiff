...
class MVDiff(nn.Module):
    ...
    def __init__(
        ...
        self.srt = SceneRepresentationTransformer(
            img_size=img_size,
            **srt_config
        )
    ...
    def forward(
        ...
        # Predict noise
        noise_pred = self.unet(noisy_images, timesteps, scene_repr_expanded, fundamental_matrices)
    ...
    def generate_views(
        ...
        shape = (B, N_out, self.unet.img_channels, input_images.shape[-2], input_images.shape[-1])
        ...
        for t in timesteps:
            ...
            scene_repr_expanded = scene_repr.unsqueeze(1).repeat(1, N_out, 1, 1).flatten(0, 1)
    ...
    def p_sample(self, x_t: torch.Tensor, noise_pred: torch.Tensor, t: int) -> torch.Tensor:
        ...
        # Compute mean
        mean = (
            torch.sqrt(alpha_cumprod_prev) * (1 - alpha_t) / (1 - alpha_cumprod_t) * x_0_pred +
