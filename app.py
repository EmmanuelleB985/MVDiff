...
class MVDiffWebApp:
    ...
    def _create_example_image(self, name: str, color: str, shape: str) -> Image.Image:
        ...
        draw.text(((256 - text_width) // 2, 220), name.upper(), 
    ...
    def generate_views(
        ...
        try:
            ...
            for i in range(num_views):
                progress(0.3 + 0.5 * i / num_views, 
                        desc=f"Generating view {i+1}/{num_views}...")
                
                # Mock view generation (rotate image)
                angle = azimuth_start + (azimuth_end - azimuth_start) * i / max(1, num_views - 1)
    ...
    def _create_grid_view(self, input_image: Image.Image, views: List[np.ndarray]) -> Image.Image:
        ...
        draw.text((width//2 - 100, 10), "Multi-View Generation", 
        ...
        for idx, (img, label) in enumerate(zip(images, labels)):
            ...
            # Add border
            draw.rectangle([x-1, y-1, x+cell_size, y+cell_size], 
                          outline='#CCCCCC', width=2)
            
            # Add label
            label_bbox = draw.textbbox((0, 0), label, font=label_font)
            label_width = label_bbox[2] - label_bbox[0]
            draw.text((x + (cell_size - label_width)//2, y + cell_size + 2), 
                     label, fill='#666666', font=label_font)
        
        return canvas
    
    def _create_camera_plot(self, num_views: int, elevation: float, radius: float) -> go.Figure:
    ...
    def _create_metrics_display(self, num_views: int, num_steps: int, 
...
def create_interface():
    ...
    with gr.Blocks(
        title="MVDiff - Multi-View Generation",
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        with gr.Accordion(" Advanced Settings", open=False):
                            elevation = gr.Slider(
                                minimum=-90, maximum=90, value=15, step=5,
                                label="Camera Elevation (degrees)"
                            )
                            azimuth_start = gr.Slider(
                                minimum=0, maximum=360, value=0, step=15,
                                label="Azimuth Start (degrees)"
                            )
                            azimuth_end = gr.Slider(
                                minimum=0, maximum=360, value=360, step=15,
        ...
        generate_btn.click(
            ...
            inputs=[
                input_image, num_views, num_steps, guidance_scale,
                elevation, azimuth_start, azimuth_end, radius, seed
            ],
            outputs=[
                output_grid, camera_plot, mesh_preview, 
                metrics_display, status_text
            ]
        )
