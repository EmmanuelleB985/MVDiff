...
from evaluation.metrics import compute_psnr, compute_ssim, compute_lpips
...
class Evaluator:
    ...
    def evaluate(self, num_samples: int = None) -> dict:
        ...
        num_batches = len(self.dataloader) if num_samples is None else \
                     min(num_samples // 4, len(self.dataloader))
