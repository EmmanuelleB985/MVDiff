    ...
    def _process_object(self, obj_info: Dict, split_dir: Path) -> Optional[Dict]:
        ...
        try:
            ...
            if self.render_views:
                ...
                images = self._render_views(obj_info['mesh_file'], obj_info['has_texture'])
            else:
            ...
            if obj_info['has_texture']:
                src_texture = Path(obj_info['object_path']) / "materials" / "textures" / "texture.png"
        ...
        try:
            ...
            for i in range(self.NUM_VIEWS):
                ...
                try:
                    ...
                    # Render
                    image_data = scene.save_image(resolution=[self.IMAGE_SIZE, self.IMAGE_SIZE])
            ...
            return [Image.new('RGB', (self.IMAGE_SIZE, self.IMAGE_SIZE), 'white') 
...
Example usage:
  python prepare_gso.py \\
    ...
    parser.add_argument(
        '--gso_dir', type=str, required=True,
        help='Path to GSO dataset directory'
    )
    parser.add_argument(
        '--output_dir', type=str, default='data/gso_mvdiff',
    ...
    parser.add_argument(
        '--render_views', action='store_true', default=True,
        help='Render views from 3D models (default: True)'
    )
    parser.add_argument(
        '--no_render', action='store_false', dest='render_views',
