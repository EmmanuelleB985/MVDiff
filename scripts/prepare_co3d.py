...
class CO3DPreparer:
    ...
    def _process_sequence(self, sequence: Dict, split_dir: Path) -> Optional[Dict]:
        ...
        try:
            ...
            if len(frame_paths) > self.TARGET_VIEWS:
                # Sample uniformly
                indices = np.linspace(
                    0, len(frame_paths) - 1, self.TARGET_VIEWS, dtype=int
                )
            ...
            if cameras_file.exists():
                cameras = self._process_cameras(
                    cameras_file,
                    indices if len(frame_paths) > self.TARGET_VIEWS else None,
                )
    ...
    def _process_cameras(
        self, cameras_file: Path, indices: Optional[List[int]] = None
    ) -> Dict:
    ...
    def _generate_statistics(self):
        ...
        for split, split_stats in stats["splits"].items():
            logger.info(
                f"  {split:5}: {split_stats['num_sequences']:6,} sequences, "
...
Example usage:
  python prepare_co3d.py \\
    ...
    parser.add_argument(
        "--co3d_dir", type=str, required=True, help="Path to CO3D dataset directory"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="data/co3d_mvdiff",
