...
class ShapeNetPreparer:
    ...
    def __init__(
        ...
        if not self.shapenet_dir.exists():
            raise FileNotFoundError(f"ShapeNet directory not found: {self.shapenet_dir}")
        if not self.rendering_dir.exists():
            raise FileNotFoundError(f"Rendering directory not found: {self.rendering_dir}")
    ...
    def _collect_samples(self) -> List[Dict]:
        ...
        for cat_id, cat_name in tqdm(self.CATEGORIES.items(), desc="Categories"):
            ...
            if not cat_rendering_dir.exists():
                logger.warning(f"Category {cat_name} ({cat_id}) not found in renderings")
                continue
            ...
            for obj_id in tqdm(object_ids, desc=f"  {cat_name}", leave=False):
                ...
                if len(images) >= self.NUM_VIEWS:
                    ...
                    model_path = self.shapenet_dir / cat_id / obj_id / "models" / "model_normalized.obj"
...
Example usage:
  python prepare_shapenet.py \\
    ...
    parser.add_argument(
        '--shapenet_dir', type=str, required=True,
        help='Path to ShapeNetCore.v2 directory'
    )
    parser.add_argument(
        '--rendering_dir', type=str, required=True,
        help='Path to ShapeNetRendering directory'
    )
    parser.add_argument(
        '--output_dir', type=str, default='data/shapenet_mvdiff',
    ...
    if args.categories:
        preparer.CATEGORIES = {
            k: v for k, v in preparer.CATEGORIES.items()
            if v in args.categories
        }
