...
def create_sample_image(name='object', size=256):
    ...
    if name == 'chair':
        # Draw chair
        draw.rectangle([size//3, size//4, size*2//3, size*3//4], fill='brown', outline='black', width=2)
        draw.rectangle([size//3, size//2, size*2//3, size*2//3], fill='brown', outline='black', width=2)
        for x in [size//3 + 10, size*2//3 - 10]:
            draw.line([x, size//2, x, size*5//6], fill='black', width=3)
    elif name == 'car':
        # Draw car
        draw.rectangle([size//4, size//2, size*3//4, size*2//3], fill='red', outline='black', width=2)
        draw.polygon([(size//3, size//2), (size*2//3, size//2),
                     (size*3//5, size//3), (size*2//5, size//3)], fill='red', outline='black')
        for x in [size//3, size*2//3 - 20]:
            draw.ellipse([x-15, size*2//3-10, x+15, size*2//3+20], fill='black')
    else:
        # Draw generic shape
        draw.ellipse([size//4, size//4, size*3//4, size*3//4], fill='blue', outline='black', width=2)
