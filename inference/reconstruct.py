...
class Reconstructor:
    ...
    def reconstruct_mvs(
        self, images: List[np.ndarray], poses: torch.Tensor, intrinsics: torch.Tensor
    ) -> dict:
        """
        ...
        return {"points": point_cloud, "colors": colors, "method": "mvs"}
    ...
    def _backproject(
        self, image: np.ndarray, depth: np.ndarray, K: np.ndarray, pose: np.ndarray
    ) -> tuple:
        ...
        points_cam = np.stack([xx * depth, yy * depth, depth], axis=-1)

        # Transform to world space
        points_cam_homo = np.concatenate(
            [points_cam.reshape(-1, 3), np.ones((h * w, 1))], axis=1
        )
    ...
    def reconstruct_nerf(
        ...
        voxel_grid = (
            voxel_grid > np.percentile(voxel_grid[voxel_grid > 0], 50)
        ).astype(float)
