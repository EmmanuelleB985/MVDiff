...
class ViewGenerator:
    ...
    def generate_camera_poses(
        self, num_views: int, radius: float = 2.5, elevation: float = 0.3
        ...
        K = torch.tensor(
            [[focal, 0, 128], [0, focal, 128], [0, 0, 1]], dtype=torch.float32
        )
    ...
    @torch.no_grad()
    def generate_views(
        self, input_image: Image.Image, num_views: int = 16, num_steps: int = 50
    ) -> List[np.ndarray]:
        ...
        generated_views = self.model.generate_views(
            input_images, input_pose, target_poses, num_inference_steps=num_steps
        )
